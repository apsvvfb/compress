...
for line in io.lines(testAnnotationPath) do
	...
	if (i == batchSize) then
		...
		output,score_tmp = nil,nil
		bactchseqLens, feattemp, labelbatch, input, targets = nil, nil, nil, nil, nil
		collectgarbage()
		--for new batch
		bactchseqLenMax = 0
...
myFile:close()
scores, score_tmp, scorefile,myFile,input,targets,output=nil,nil,nil,nil,nil,nil,nil
--model:clearState()
end
