...
def draw(type,hidden,id,x_e,lr):
	inpath="batch5_hiddensize256to%d_initialized/%s_%d/" %(hidden,type,id)
	inputfile_t="%s/logTrain_h%d_%s_%d" %(inpath,hidden,type,id)
        imgname='%s_trainnum%d.png' %(type,id)
        plttitle='%s_trainnum%d' %(type,id)
...
if __name__ == "__main__":
	types=["DistKLDivCriterion","MSECriterion"]
	lr=0.005
